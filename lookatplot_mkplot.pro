pro lookatplot_mkplot,fdata,fsim,params,obspars,_extra=_extra,eps=eps
   ...
   for i=0,s[1]-1 do begin
      for j=0,s[2]-1 do begin
         mom1sim[i,j]=mean(v1[where(fsim[i,j,*] eq max(fsim[i,j,*]))])
         mom1data[i,j]=mean(v1[where(fdata[i,j,*] eq max(fdata[i,j,*]))])
      endfor
   endfor
   mom1sim[where(mom0sim lt 0.1*max(mom0sim))] =-10000
   mom1data[where(mom0data lt 0.1*max(mom0data))] =-10000
   pvdcubedata=fdata*0.0
   pvdcubesim=fsim*0.0
   for i=0,s[3]-1 do pvdcubedata[*,*,i]=rot(fdata[*,*,i],r[1]-90,/interp)
   for i=0,s[3]-1 do pvdcubesim[*,*,i]=rot(fsim[*,*,i],r[1]-90,/interp)
   pvddata=reform(pvdcubedata[*,s[2]/2.,*])
   pvdsim=reform(pvdcubesim[*,s[2]/2.,*])
;;;
   ...
   contour,mom0sim,x1,x1,/overplot,color=250,nlevels=10,levels=max(mom0sim,/nan)*((findgen(8)+2)/10.)
   ...
   contour,pvdsim,x1,v1,levels=max(pvdsim)*((findgen(9)+3)/10.),/overplot,color=250
